...
#define MAX_DATABLOCKS_COUNT 1024000
#define MAX_FILES_COUNT 32
#define FILENAME_LENGTH 128
#define PATH_TO_LLFSFILE "/home/taqtaq11/ClionProjects/LLFileSystem/fs"
...
int writeFileDescriptor(int num) {
    FILE *f = fopen(PATH_TO_LLFSFILE, "r+");
    fseek(f, num * sizeof(fileDescriptor), SEEK_SET);
    fwrite(&fds[num], sizeof(fileDescriptor), 1, f);
    fclose(f);
    return 0;
}

int writeDataBlocks(fileDescriptor* fd) {
    int i = fd->startingDataBlock;
    FILE *f = fopen(PATH_TO_LLFSFILE, "r+");

    while (i != -1) {
        fseek(f, sizeof(fileDescriptor) * MAX_FILES_COUNT + sizeof(int) * i, SEEK_SET);
...
int writeData(fileDescriptor* fd, const char* data, int size, int offset) {
    if (size == 0) return 0;
    FILE *f = fopen(PATH_TO_LLFSFILE, "r+");
    ...
    int headers_offset = sizeof(fileDescriptor) * MAX_FILES_COUNT + sizeof(int) * MAX_DATABLOCKS_COUNT;
...
int readData(fileDescriptor* fd, char** data) {
    if (fd == NULL) return -1;
    FILE *f = fopen(PATH_TO_LLFSFILE, "r");
    char *buf = NULL;
...
int getFileDescriptor(const char* path, fileDescriptor** fd) {
...
int deleteFile(const char* path) {
    fileDescriptor *fileNode, *dirNode;
    char* data;
    char* preparedData;
    char *dir = getDirName(path);

    int dirNodeNum = getFileDescriptor(dir, &dirNode);
    int fNodeNum = getFileDescriptor(path, &fileNode);
    ...
    writeFileDescriptor(dirNodeNum);
...
static void* llInit(struct fuse_conn_info* fi) {
    ...
    char* buf = (char*)malloc(sizeof(fileDescriptor));
    memset(buf, '\0', sizeof(fileDescriptor));
    int i = 0;
...
static int llGetattr(const char* path, struct stat* stbuf) {
    ...
    fileDescriptor *fd;
    if (getFileDescriptor(path, &fd) == -1)
        res = -ENOENT;
...
static int llCreate(const char* path, mode_t mode, struct fuse_file_info* finfo) {
    ...
    int nodeNum = getFileDescriptor(dir, &fd);
...
static int llReaddir(const char* path, void* buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info* fi) {
    ...
    fileDescriptor *fd;
    int nodeNum = getFileDescriptor(path, &fd);
